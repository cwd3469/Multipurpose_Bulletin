import WDetailModal, {
  ...
  WSupportDetailModalTypeQueue,
} from '@components/common/modal/WDetailModal';
import WErrorPage from '@components/common/modal/WErrorPage';
import { useSupportQueueDetail } from '@hooks/api/hospitalSupport/supportQueue';
...
const SupportQueueDetail = (props: QueueDetailModalType) => {
  const toast = useToastContext();
  const msg = useCodeMsgBundle();
  const router = useRouter();
  const { data: detailData, isError } = useSupportQueueDetail(props.ulid);
  ...
  if (detailData) {
    if (detailData.data.code !== '0000') {
      ...
      const res: WSupportDetailModalTypeQueue = detailData.data.data;
...
export default SupportQueueDetail;
