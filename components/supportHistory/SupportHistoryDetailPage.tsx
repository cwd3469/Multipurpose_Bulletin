...
import useSupportHistoryDetailTreat from './hooks/useSupportHistoryDetailTreat';
import WaitingUlidContext from '@components/waitingRoom/context/WaitingUlidContext';

const SupportHistoryDetailPage = (props: { ulid: string }) => {
  const { query } = useRouter();
  const { ulid, setInUlid } = useContext(WaitingUlidContext);
  const { historyDetailData, isLoading } = useSupportHistoryDetailTreat(
    props.ulid,
  );
  ...
  useEffect(() => {
    if (historyDetailData) {
      setHeaderData({
        ...
        closedAt: historyDetailData.completedAt,
        registrationUlid: historyDetailData.registrationUlid,
        nameKo: historyDetailData.patientNameKo,
        age: historyDetailData.patientAge,
        gender: historyDetailData.gender,
        mobileNum: historyDetailData.patientMobileNUm,
      ...
      setPatientMessage(historyDetailData.messageToPatient);
    }
    return () => {
...
export default SupportHistoryDetailPage;
